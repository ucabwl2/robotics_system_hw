...
double a[6] = {0.0, 0.155, 0.135, 0.0, 0.0};
double alpha[6] = {M_PI_2, M_PI, M_PI, 3.0 * M_PI_2, 0.0};
double d[6] = {0.147, 0.0, 0.0, 0.0, 0.218};
//double theta[6] = {0.0, M_PI_2 - 1.08, 0.0 - 2.49 , 3.0 * M_PI_2 - 1.82, 0.0};
double theta[6] = {0.0, M_PI_2, 0.0 , 3.0 * M_PI_2, 0.0};
...
void forward_kinematic(const sensor_msgs::JointState::ConstPtr& joint_msg, tf2_ros::TransformBroadcaster br){
    ...
    transform[0].child_frame_id = "arm5d_link_1";
    transform[1].child_frame_id = "arm5d_link_2";
    transform[2].child_frame_id = "arm5d_link_3";
    transform[3].child_frame_id = "arm5d_link_4";
    transform[4].child_frame_id = "arm5d_link_5";

    for (int i = 0; i < 5; i++)
    {
        //the frame 0 and frame 2 are reversed, because in the '.xacro' file the z axes are reversed 
        if ((i == 0) || (i == 2))
